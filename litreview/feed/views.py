...
from .forms import TicketForm, ReviewForm, FollowUsersForm, UserSearchForm
...
def follow_users(request):
    ...
    if request.method == 'POST':
        if 'username' in request.POST:  # searching for users
            ...
            if search_form.is_valid():
                searched_users = User.objects.filter(username__icontains=search_form.cleaned_data['username'])
                search_performed = True  
        elif 'follow' in request.POST:  # following a user
            followed_user = User.objects.get(id=request.POST['follow'])
            user_follows, created = UserFollows.objects.get_or_create(user=request.user, followed_user=followed_user)
    ...
    return render(request, 'feed/follow_users.html', context={'search_form': search_form, 'searched_users': searched_users, 'following': following, 'followers': followers, 'search_performed': search_performed})
